...
aead_cipher_ctx_set_key(cipher_ctx_t *cipher_ctx, int enc)
{
    ...
    int err = CNWLibsscrypto_crypto_hkdf(md,
...
void
CNWLibsscrypto_AEAD_context_init(cipher_t *cipher, cipher_ctx_t *cipher_ctx, int enc)
{
    ...
    if (enc) {
        CNWLibsscrypto_random_bytes(cipher_ctx->salt, cipher->key_len);
    }
}

void
CNWLibsscrypto_AEAD_context_release(cipher_ctx_t *cipher_ctx)
{
...
int
CNWLibsscrypto_AEAD_encrypt_all(buffer_t *plaintext, cipher_t *cipher, size_t capacity)
{
    cipher_ctx_t cipher_ctx;
    CNWLibsscrypto_AEAD_context_init(cipher, &cipher_ctx, 1);
    ...
    CNWLibsscrypto_buffer_realloc(&tmp, salt_len + tag_len + plaintext->len, capacity);
    ...
    CNWLibsscrypto_AEAD_context_release(&cipher_ctx);
    ...
    CNWLibsscrypto_buffer_realloc(plaintext, salt_len + ciphertext->len, capacity);
...
int
CNWLibsscrypto_AEAD_decrypt_all(buffer_t *ciphertext, cipher_t *cipher, size_t capacity)
{
    size_t salt_len = cipher->key_len;
    size_t tag_len  = cipher->tag_len;
    int err         = CRYPTO_OK;
    ...
    cipher_ctx_t cipher_ctx;
    CNWLibsscrypto_AEAD_context_init(cipher, &cipher_ctx, 0);
...
/* TCP */
int
CNWLibsscrypto_AEAD_encrypt(buffer_t *plaintext, cipher_ctx_t *cipher_ctx, size_t capacity)
{
    if (cipher_ctx == NULL)
        return CRYPTO_ERROR;
    ...
    CNWLibsscrypto_buffer_realloc(&tmp, out_len, capacity);
    ciphertext      = &tmp;
    ...
    CNWLibsscrypto_buffer_realloc(plaintext, ciphertext->len, capacity);
...
int
CNWLibsscrypto_AEAD_decrypt(buffer_t *ciphertext, cipher_ctx_t *cipher_ctx, size_t capacity)
{
    int err             = CRYPTO_OK;
    static buffer_t tmp = { 0, 0, 0, NULL };
    ...
    if (cipher_ctx->chunk == NULL) {
        ...
        CNWLibsscrypto_buffer_alloc(cipher_ctx->chunk, capacity);
    }
    
    CNWLibsscrypto_buffer_realloc(cipher_ctx->chunk,
    ...
    CNWLibsscrypto_buffer_realloc(&tmp, cipher_ctx->chunk->len, capacity);
    buffer_t *plaintext = &tmp;
...
aead_key_init(int method, const char *pass, const char *key)
{
    ...
    if (key != NULL)
        cipher->key_len = CNWLibsscrypto_crypto_parse_key(key, cipher->key,
                                           supported_aead_ciphers_key_size[method]);
    else
        cipher->key_len = CNWLibsscrypto_crypto_derive_key(pass, cipher->key,
                                            supported_aead_ciphers_key_size[method]);
...
cipher_t *
CNWLibsscrypto_AEAD_init(const char *pass, const char *key, const char *method)
{
    int m = AES128GCM;
    if (method != NULL) {
