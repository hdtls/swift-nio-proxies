...
int
CNWLibsscrypto_buffer_alloc(buffer_t *ptr, size_t capacity)
{
...
int
CNWLibsscrypto_buffer_realloc(buffer_t *ptr, size_t len, size_t capacity)
{
    if (ptr == NULL)
        return -1;
...
void
CNWLibsscrypto_buffer_free(buffer_t *ptr)
{
    if (ptr == NULL)
        return;
    ptr->idx      = 0;
...
int
CNWLibsscrypto_buffer_prepend(buffer_t *dst, buffer_t *src, size_t capacity)
{
    CNWLibsscrypto_buffer_realloc(dst, dst->len + src->len, capacity);
...
int
CNWLibsscrypto_random_bytes(void *output, int len)
{
...
crypto_t *
CNWLibsscrypto_crypto_init(const char *password, const char *key, const char *method)
{
    int i, m = -1;
    ...
    if (method != NULL) {
        ...
        if (m != -1) {
            ...
            cipher_t *cipher = CNWLibsscrypto_stream_init(password, key, method);
            if (cipher == NULL)
                return NULL;
            crypto_t *crypto = (crypto_t *)ss_malloc(sizeof(crypto_t));
            crypto_t tmp     = {
                .cipher      = cipher,
                .encrypt_all = &CNWLibsscrypto_stream_encrypt_all,
                .decrypt_all = &CNWLibsscrypto_stream_decrypt_all,
                .encrypt     = &CNWLibsscrypto_stream_encrypt,
                .decrypt     = &CNWLibsscrypto_stream_decrypt,
                .ctx_init    = &CNWLibsscrypto_stream_context_init,
                .ctx_release = &CNWLibsscrypto_stream_context_release,
            };
        ...
        if (m != -1) {
            cipher_t *cipher = CNWLibsscrypto_AEAD_init(password, key, method);
            if (cipher == NULL)
                return NULL;
            crypto_t *crypto = (crypto_t *)ss_malloc(sizeof(crypto_t));
            crypto_t tmp     = {
                .cipher      = cipher,
                .encrypt_all = &CNWLibsscrypto_AEAD_encrypt_all,
                .decrypt_all = &CNWLibsscrypto_AEAD_decrypt_all,
                .encrypt     = &CNWLibsscrypto_AEAD_encrypt,
                .decrypt     = &CNWLibsscrypto_AEAD_decrypt,
                .ctx_init    = &CNWLibsscrypto_AEAD_context_init,
                .ctx_release = &CNWLibsscrypto_AEAD_context_release,
            };
            memcpy(crypto, &tmp, sizeof(crypto_t));
            return crypto;
        }
...
int
CNWLibsscrypto_crypto_derive_key(const char *pass, uint8_t *key, size_t key_len)
{
    size_t datal;
...
int
CNWLibsscrypto_crypto_hkdf(const mbedtls_md_info_t *md, const unsigned char *salt,
            ...
            int okm_len)
{
    ...
    return CNWLibsscrypto_crypto_hkdf_extract(md, salt, salt_len, ikm, ikm_len, prk) ||
    CNWLibsscrypto_crypto_hkdf_expand(md, prk, mbedtls_md_get_size(md), info, info_len,
                       okm, okm_len);
}

/* HKDF-Extract(salt, IKM) -> PRK */
int
CNWLibsscrypto_crypto_hkdf_extract(const mbedtls_md_info_t *md, const unsigned char *salt,
                    int salt_len, const unsigned char *ikm, int ikm_len,
                    unsigned char *prk)
{
    int hash_len;
...
int
CNWLibsscrypto_crypto_hkdf_expand(const mbedtls_md_info_t *md, const unsigned char *prk,
...
int
CNWLibsscrypto_crypto_parse_key(const char *base64, uint8_t *key, size_t key_len)
{
    size_t base64_len = strlen(base64);
    int out_len       = BASE64_SIZE(base64_len);
    uint8_t out[out_len];
    
    out_len = CNWLibsscrypto_base64_decode(out, base64, out_len);
    ...
    CNWLibsscrypto_random_bytes(key, key_len);
    CNWLibsscrypto_base64_encode(out_key, out_len, key, key_len);
