...
void
CNWLibsscrypto_stream_context_release(cipher_ctx_t *cipher_ctx)
{
    if (cipher_ctx->chunk != NULL) {
        CNWLibsscrypto_buffer_free(cipher_ctx->chunk);
...
int
CNWLibsscrypto_stream_encrypt_all(buffer_t *plaintext, cipher_t *cipher, size_t capacity)
{
    cipher_ctx_t cipher_ctx;
    CNWLibsscrypto_stream_context_init(cipher, &cipher_ctx, 1);
    
    size_t nonce_len = cipher->nonce_len;
    int err          = CRYPTO_OK;
    
    static buffer_t tmp = { 0, 0, 0, NULL };
    CNWLibsscrypto_buffer_realloc(&tmp, nonce_len + plaintext->len, capacity);
    ...
    CNWLibsscrypto_stream_context_release(&cipher_ctx);
    ...
    CNWLibsscrypto_buffer_realloc(plaintext, nonce_len + ciphertext->len, capacity);
...
int
CNWLibsscrypto_stream_encrypt(buffer_t *plaintext, cipher_ctx_t *cipher_ctx, size_t capacity)
{
    if (cipher_ctx == NULL)
        return CRYPTO_ERROR;
    ...
    if (cipher->method >= SALSA20) {
        ...
        CNWLibsscrypto_buffer_realloc(ciphertext, nonce_len + (padding + ciphertext->len) * 2, capacity);
        if (padding) {
            CNWLibsscrypto_buffer_realloc(plaintext, plaintext->len + padding, capacity);
...
int
CNWLibsscrypto_stream_decrypt_all(buffer_t *ciphertext, cipher_t *cipher, size_t capacity)
{
    size_t nonce_len = cipher->nonce_len;
    int err          = CRYPTO_OK;
    ...
    cipher_ctx_t cipher_ctx;
    CNWLibsscrypto_stream_context_init(cipher, &cipher_ctx, 0);
    
    static buffer_t tmp = { 0, 0, 0, NULL };
    CNWLibsscrypto_buffer_realloc(&tmp, ciphertext->len, capacity);
    ...
    CNWLibsscrypto_buffer_realloc(ciphertext, plaintext->len, capacity);
...
int
CNWLibsscrypto_stream_decrypt(buffer_t *ciphertext, cipher_ctx_t *cipher_ctx, size_t capacity)
{
    if (cipher_ctx == NULL)
        return CRYPTO_ERROR;
    ...
    if (!cipher_ctx->init) {
        if (cipher_ctx->chunk == NULL) {
            ...
            CNWLibsscrypto_buffer_alloc(cipher_ctx->chunk, cipher->nonce_len);
        }
    ...
    if (cipher->method >= SALSA20) {
        int padding = cipher_ctx->counter % SODIUM_BLOCK_SIZE;
        CNWLibsscrypto_buffer_realloc(plaintext, (plaintext->len + padding) * 2, capacity);
        
        if (padding) {
            CNWLibsscrypto_buffer_realloc(ciphertext, ciphertext->len + padding, capacity);
...
void
CNWLibsscrypto_stream_context_init(cipher_t *cipher, cipher_ctx_t *cipher_ctx, int enc)
{
    ...
    if (enc) {
        CNWLibsscrypto_random_bytes(cipher_ctx->nonce, cipher->nonce_len);
    }
}
...
stream_key_init(int method, const char *pass, const char *key)
{
    ...
    if (key != NULL)
        cipher->key_len = CNWLibsscrypto_crypto_parse_key(key, cipher->key, cipher_key_size(cipher));
    else
        cipher->key_len = CNWLibsscrypto_crypto_derive_key(pass, cipher->key, cipher_key_size(cipher));
...
cipher_t *
CNWLibsscrypto_stream_init(const char *pass, const char *key, const char *method)
{
    int m = TABLE;
    if (method != NULL) {
