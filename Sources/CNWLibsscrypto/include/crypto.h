...
int CNWLibsscrypto_buffer_alloc(buffer_t *, size_t);
int CNWLibsscrypto_buffer_realloc(buffer_t *, size_t, size_t);
int CNWLibsscrypto_buffer_prepend(buffer_t *, buffer_t *, size_t);
void CNWLibsscrypto_buffer_free(buffer_t *);
int CNWLibsscrypto_random_bytes(void *, int);

crypto_t *CNWLibsscrypto_crypto_init(const char *pwd, const char *key, const char *);
unsigned char *crypto_md5(const unsigned char *, size_t, unsigned char *);

int CNWLibsscrypto_crypto_derive_key(const char *, uint8_t *, size_t);
int CNWLibsscrypto_crypto_parse_key(const char *, uint8_t *, size_t);
int CNWLibsscrypto_crypto_hkdf(const mbedtls_md_info_t *md, const unsigned char *salt,
                int salt_len, const unsigned char *ikm, int ikm_len,
                const unsigned char *info, int info_len, unsigned char *okm,
                int okm_len);
int CNWLibsscrypto_crypto_hkdf_extract(const mbedtls_md_info_t *md, const unsigned char *salt,
                        int salt_len, const unsigned char *ikm, int ikm_len,
                        unsigned char *prk);
int CNWLibsscrypto_crypto_hkdf_expand(const mbedtls_md_info_t *md, const unsigned char *prk,
