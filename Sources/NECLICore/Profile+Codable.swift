...
extension Profile: Codable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    let policyGroups =
      try container.decodeIfPresent([AnyPolicyGroup].self, forKey: .policyGroups)?.map {
        $0.base
      } ?? []
  ...
  public func encode(to encoder: Encoder) throws {
    ...
    try container.encode(policyGroups.map(AnyPolicyGroup.init), forKey: .policyGroups)
  }
}
