...
public final class SSClientProxyHandler: ChannelDuplexHandler {
...
//        self.decipher = try security.makeDecryptor()
    ...
    public func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        guard var bytesCopy = unwrapped.readBytes(length: unwrapped.readableBytes) else {
            context.fireChannelRead(data)
            return
        }

        do {
            var byteBuffer = context.channel.allocator.buffer(capacity: bytesCopy.count)
            
//            byteBuffer.writeBytes(try decipher.update(bytesCopy))
