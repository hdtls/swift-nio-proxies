...
public class CertificateStore {
    ...
    public convenience init(organization: String, commonName: String) {
        ...
        CNIOBoringSSL_X509_gmtime_adj(CNIOBoringSSL_X509_get_notAfter(certificate), 86400 * 365 * 10)
        
        CNIOBoringSSL_X509_set_issuer_name(certificate, name)
        
        CNIOBoringSSL_X509_set_pubkey(certificate, privateKey)
        
        CertificateStore.boringSSLX509AddExtension(x509: certificate, nid: NID_basic_constraints, value: "critical, CA:TRUE")
        CertificateStore.boringSSLX509AddExtension(x509: certificate, nid: NID_ext_key_usage, value: "serverAuth")
        CertificateStore.boringSSLX509AddExtension(x509: certificate, nid: NID_key_usage, value: "critical, keyCertSign, cRLSign")
        CertificateStore.boringSSLX509AddExtension(x509: certificate, nid: NID_subject_key_identifier, value: "hash")
    ...
    func generateCertificate(commonName: String, organization: String? = nil, subjectAltNames: [String], pubkey: UnsafeMutablePointer<EVP_PKEY>) -> OpaquePointer {
        /* Sign with the CA. */
        let certificate = CNIOBoringSSL_X509_new()!
        ...
        CNIOBoringSSL_X509_set_issuer_name(certificate, CNIOBoringSSL_X509_get_subject_name(self.certificate))
        ...
        CertificateStore.boringSSLX509AddExtension(x509: certificate, nid: NID_subject_alt_name, value: subjectAltName)
    ...
    static func exportP12Bundle(passphrase: String?, name: String? = nil, certificate: OpaquePointer, privateKey: UnsafeMutablePointer<EVP_PKEY>) throws -> NIOSSLPKCS12Bundle {
        let certificateStore = CertificateStore(certificate: certificate, privateKey: privateKey)
        let buffer = try CertificateStore.convertP12ToDERBytes(certificateStore.exportP12Bundle(passphrase: passphrase, name: name))
    ...
    func exportP12Bundle(passphrase: String? = nil, name: String? = nil) throws -> OpaquePointer {
        ...
        if let passphrase = passphrase {
            ...
            p12 = CNIOBoringSSL_PKCS12_create(nil, name, privateKey, certificate, nil, 0, 0, 0, 0, 0)
        }
    ...
    private static func boringSSLX509AddExtension(x509: OpaquePointer, nid: CInt, value: String) {
        ...
        let ext = value.withCString { (pointer) in
            return CNIOBoringSSL_X509V3_EXT_nconf_nid(nil, &ctx, nid, UnsafeMutablePointer(mutating: pointer))
        }!
