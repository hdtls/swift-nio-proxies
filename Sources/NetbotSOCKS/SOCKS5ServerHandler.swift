...
extension SOCKS5ServerHandler {
    ...
    private func evaluateClientRequestAndReplies(context: ChannelHandlerContext) throws {
        ...
        let completion: (NetAddress) -> EventLoopFuture<Channel> = { _ in context.eventLoop.makeCompletedFuture(.failure(SOCKSError.unexpectedRead)) }
    ...
    private func handleGlue(peer: Channel, context: ChannelHandlerContext) {
        ...
        let response: Response = .init(reply: .succeeded, boundAddress: .socketAddress(peer.remoteAddress!))
