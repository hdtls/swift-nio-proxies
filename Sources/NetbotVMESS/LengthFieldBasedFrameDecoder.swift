...
final public class LengthFieldBasedFrameDecoder: ByteToMessageDecoder {
    ...
    private func parseLengthField(context: ChannelHandlerContext, buffer: inout ByteBuffer) throws -> (UInt16, Int)? {
        guard size == nil else {
            return size
        }
    ...
    private func parseFrame(context: ChannelHandlerContext, buffer: inout ByteBuffer) throws -> ByteBuffer? {
        // TCP
        ...
        let combined = nonce + buffer.readBytes(length: Int(size.frameLength))!.dropLast(size.padding)
    ...
    private func parseLastFrame(context: ChannelHandlerContext, buffer: inout ByteBuffer) -> ByteBuffer {
