//===----------------------------------------------------------------------===//
//
// This source file is part of the Netbot open source project
//
// Copyright (c) 2022 Junfeng Zhang and the Netbot project authors
// Licensed under Apache License v2.0
//
// See LICENSE for license information
// See CONTRIBUTORS.txt for the list of Netbot project authors
//
// SPDX-License-Identifier: Apache-2.0
//
//===----------------------------------------------------------------------===//

import XCTest

@testable import NEHTTPMitM

final class CertCacheTests: XCTestCase {

  lazy var cache = try! CertCache(
    base64EncodedP12String: base64EncodedP12String,
    passphrase: passphrase
  )

  lazy var base64EncodedP12String: String =
    ""
  let passphrase = "Y49KEDR7"

  override func setUp() {
    cache.setUpMitMHosts([])
    cache.removeAllValues()
  }

  func testInitializeWithWrongPassphrase() {
    XCTAssertThrowsError(
      try CertCache(base64EncodedP12String: base64EncodedP12String, passphrase: "wrong passphrase")
    )
  }

  func testInitializeWithInvalidP12String() {
    XCTAssertThrowsError(
      try CertCache(base64EncodedP12String: "base64EncodedP12String", passphrase: passphrase)
    )
  }

  func testDelayCertificateCreation() throws {
    cache.setUpMitMHosts(["*.swift.org"])
    XCTAssertTrue(cache.isEmpty)
    XCTAssertEqual(cache.hostnames, ["*.swift.org"])

    cache.setUpMitMHosts(["*.swift.org", "swift.org"])
    XCTAssertTrue(cache.isEmpty)
    XCTAssertEqual(cache.hostnames, ["*.swift.org", "swift.org"])

    _ = try cache.value(forKey: "swift.org")
    XCTAssertEqual(cache.count, 1)
    XCTAssertFalse(cache.isEmpty)

    _ = try cache.value(forKey: "*.swift.org")
    XCTAssertEqual(cache.count, 2)
    XCTAssertFalse(cache.isEmpty)
  }

  func testSetMitMHostnames() throws {
    cache.setUpMitMHosts(["*.swift.org"])
    XCTAssertTrue(cache.isEmpty)
    XCTAssertEqual(cache.hostnames, ["*.swift.org"])

    _ = try cache.value(forKey: "*.swift.org")
    XCTAssertEqual(cache.count, 1)
    XCTAssertFalse(cache.isEmpty)

    cache.setUpMitMHosts(["*.swift.org", "swift.org"])
    _ = try cache.value(forKey: "swift.org")
    XCTAssertEqual(cache.hostnames, ["*.swift.org", "swift.org"])
    XCTAssertEqual(cache.count, 2)

    cache.setUpMitMHosts(["*.swift.org"])
    XCTAssertEqual(cache.hostnames, ["*.swift.org"])
    XCTAssertEqual(cache.count, 1)

    cache.setUpMitMHosts([])
    XCTAssertEqual(cache.count, 0)
    XCTAssertTrue(cache.isEmpty)
  }

  func testFindCertWithServerHostname() throws {
    var entry = try cache.value(forKey: "swift.org")
    XCTAssertNil(entry)

    cache.setUpMitMHosts(["swift.org"])
    entry = try cache.value(forKey: "swift.org")
    XCTAssertNotNil(entry)

    entry = try cache.value(forKey: "*.swift.org")
    XCTAssertNil(entry)
  }

  func testRemoveCacheEntry() throws {
    XCTAssertNil(cache.removeValue(forKey: "swift.org"))
    XCTAssertEqual(cache.count, 0)
    XCTAssertTrue(cache.isEmpty)

    cache.setUpMitMHosts(["swift.org"])
    XCTAssertNil(cache.removeValue(forKey: "swift.org"))
    XCTAssertEqual(cache.count, 0)
    XCTAssertTrue(cache.isEmpty)

    _ = try cache.value(forKey: "swift.org")
    XCTAssertNotNil(cache.removeValue(forKey: "swift.org"))
    XCTAssertEqual(cache.count, 0)
    XCTAssertTrue(cache.isEmpty)
    XCTAssertEqual(cache.hostnames, ["swift.org"])
  }

  func testRemoveAllEntries() throws {
    cache.setUpMitMHosts(["swift.org", "*.swift.org"])
    _ = try cache.value(forKey: "swift.org")
    _ = try cache.value(forKey: "*.swift.org")
    cache.removeAllValues()
    XCTAssertEqual(cache.count, 0)
    XCTAssertTrue(cache.isEmpty)
    XCTAssertEqual(cache.hostnames, ["swift.org", "*.swift.org"])
  }
}
